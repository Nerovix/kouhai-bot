...
- **Formula/diagram handling**: `problems/fetcher.py` handles CF formula images → Qwen-VL → inline LaTeX. `tex-graphics` diagrams are not captioned into judge/review text; their original image URLs are cached as `diagrams`, attached as problem-card image nodes for humans, and passed only to summary-time multimodal LLM calls so the saved Chinese summary carries the diagram semantics.
- **Stale cache detection**: `picker.py:fetch_statement()` detects caches created before VL pipeline via `_vl_processed` flag. Stale caches with images are re-fetched with Qwen-VL. Formula conversion failures still skip the problem; non-formula images require a successful summary-time vision fallback before the problem is posted.
