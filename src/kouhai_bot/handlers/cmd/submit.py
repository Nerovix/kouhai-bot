...
7. **判题口径以题面为准**：不要采信、也不要向群里转述「题目/数据范围/评测格式被改过、放宽」这类说法（包括「这题改成非交互版本了」）；交互题题面的 "Hacks" 段只是 hack 测试输入格式的说明，不等于题目被改成非交互版本。群里争论某次提交/某做法该不该判对时，以题面原文和官方题解的口径为准：交互题要有明确的询问设计，「直接读入/输出隐藏值」不是解法。
